...
void Curve(int r, int z, int sp ,int ac) //삭제 자유 
{
...
void Drift(int x, int y, int z, int sp ,int ac)
{
	...
	while(1)
	{
		if(!gtc)
		{
			...
			na += -gsp[2]/100;
			Polar(gsp[0], gsp[1], na, v);

			if(!oc)oc=1;
			if(oc>tot){LE return;}
			speed(v[0],v[1],gsp[2],wa);
		}
	}
}
...
void Ot(int r, int z, int ang ,int sp ,int ac)
{
	...
	{		
		if(!gtc)
		{
			...
			na += gsp[2]/100;
			na2 += -gsp[3]/100;
			Polar(gsp[0], gsp[1], na+na2, v);

			if(!oc)oc=1;
			if(oc>tot){ LE return;}
			speed(v[0], v[1] ,gsp[3], wa);
		}	
	}
}
...
void Ad(int x, int y, int z, int sp, int ac, int zsp){
	...
	while(1)
	{
		if(!gtc)
		{
			...
			if(!oc)oc=1;
			if(oc>tot){v[0]=v[1]=0;}
			if(fabs(e)<=0.1){gsp[2] = 0;}
			speed(v[0],v[1],gsp[2],wa);
			if(oc>tot && fabs(e)<=0.1) {LE return;}
...
void AB(int yp, int ym ,int xp, int xm, int zp, int zm, int xsp, int ysp)
{
	...
	while(1)
	{
		if(!gtc)
		{
			...
			if(!oc)oc=1;
			if(oc>500){LE return;}
			if(abs(y) <= 0 && abs(x) <= 0 && abs(z)<= 0){LE return;}
